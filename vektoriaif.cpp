...
void RandomUnknown(int i, vector<int>&nd){
    nd.push_back(dist(mt));
    cout << i + 1 << "-am studentui"<< "Pridetas :" << nd.back() << endl;
}
...
void IvestisSksizeknown(int n, vector<int> &nd, int i)
{   
        int grade;   
                for (int j = 0; j < n; j++)
                {
...
void IvestisSKsizenotknownCorrect(int i, vector <int> &nd){
                    int grade;
        ...
        char choise;

            {
                while (true)
                {
                    ...
                    if (choise == 'Y' || choise == 'y')
                    {
                        IvestisSKsizenotknownCorrect(i, nd);
                        cin.clear();
                    }
                    else if (choise == 'N' || choise == 'n')
                    {
...
void Generatefromfile(Container&s, Container&w, Container&l, int choise) {
    ...
    if (knowhomework == 0 && Entergrades == 0)
        ...
        else if (knowhomework == 1 && Entergrades == 1)
        {
            ...
            IvestisSksizeknown(stud.n, stud.nd, i);
        }
